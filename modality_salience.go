// modality_salience.go — Attentional field scoring for modality events.
//
...
var modalityBoosts = map[string]float64{
	EventModalityInput:       1.0, // perception just happened
	EventModalityOutput:      0.8, // action was taken
	EventModalityGate:        0.3, // gate decision, lower salience
	EventModalityTransform:   0.2, // pipeline step
	EventModalityStateChange: 0.5, // module lifecycle change
	EventModalityError:       1.5, // errors are highly salient
}
...
// ModalitySalience extends the attentional field with modality event scoring.
type ModalitySalience struct {
	mu     sync.RWMutex
...
func (ms *ModalitySalience) OnEvent(eventType string, data map[string]any) {
	...
	boost, ok := modalityBoosts[eventType]
	if !ok {
...
func (ms *ModalitySalience) Decay(now time.Time, halfLife time.Duration) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *ModalitySalience) TopN(n int) []*SalienceEntry {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
...
func (ms *ModalitySalience) Snapshot() map[string]float64 {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
...
func (ms *ModalitySalience) Score(key string) float64 {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
