// modality_supervisor.go — Process supervisor for Python inference subprocesses.
package main
...
func (s *ProcessSupervisor) Register(cfg *SupervisorConfig) error {
	...
	s.processes[cfg.Name] = &ManagedModule{
		...
		Status: ModuleStatusStopped, MaxRestarts: cfg.MaxRestarts,
...
func (s *ProcessSupervisor) Start(ctx context.Context, name string) error {
	...
	if m.Status == ModuleStatusHealthy || m.Status == ModuleStatusStarting {
		s.mu.Unlock()
	...
	if err := os.MkdirAll(m.logDir, 0755); err != nil {
		s.setStatus(name, ModuleStatusCrashed, err.Error())
		return err
	}
	...
	case <-time.After(30 * time.Second):
		_ = conn.Close()
		s.setStatus(name, ModuleStatusCrashed, "ready timeout")
		return fmt.Errorf("supervisor: %s did not become ready within 30s", name)
	case <-ctx.Done():
		_ = conn.Close()
		s.setStatus(name, ModuleStatusStopped, "cancelled")
		return ctx.Err()
	}
	hctx, hcancel := context.WithCancel(context.Background())
	s.mu.Lock()
	m.Conn, m.PID = conn, conn.PID()
	m.StartedAt, m.Status, m.LastError, m.cancel = time.Now(), ModuleStatusHealthy, "", hcancel
	s.mu.Unlock()
...
func (s *ProcessSupervisor) Stop(_ context.Context, name string) error {
	...
	s.mu.Lock()
	m.Conn, m.PID, m.Status, m.cancel = nil, 0, ModuleStatusStopped, nil
	s.mu.Unlock()
	return closeErr
}
...
func (s *ProcessSupervisor) Status(name string) (*ModuleState, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	m, ok := s.processes[name]
	if !ok {
...
func (s *ProcessSupervisor) healthLoop(ctx context.Context, name string, interval time.Duration) {
	...
	for {
		...
		if _, err := conn.HealthCheck(2 * time.Second); err == nil {
			if failures > 0 {
				failures = 0
				s.setStatus(name, ModuleStatusHealthy, "")
			}
			continue
		} else {
			failures++
		...
		if failures == 2 {
			s.setStatus(name, ModuleStatusDegraded, "health check failing")
		}
		if failures >= 3 {
			s.mu.RLock()
			...
			s.mu.RUnlock()
			if restarts >= maxR {
				s.setStatus(name, ModuleStatusDegraded, "max restarts exceeded")
				return
			}
