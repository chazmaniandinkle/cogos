// modality_bus.go — ModalityBus orchestrates modality modules.
//
...
//
// Translated from Python POC (mod3/bus.py) into idiomatic Go.
package main
...
// ModalityBus orchestrates modality modules. It is the sensorimotor
// boundary between the agent and its environment.
type ModalityBus struct {
	mu        sync.RWMutex
	modules   map[ModalityType]ModalityModule
...
// NewModalityBus creates a bus with default configuration.
func NewModalityBus() *ModalityBus {
	return &ModalityBus{
		modules:   make(map[ModalityType]ModalityModule),
...
func (b *ModalityBus) Register(module ModalityModule) error {
	b.mu.Lock()
	defer b.mu.Unlock()
	mt := module.Type()
...
func (b *ModalityBus) Perceive(raw []byte, modality ModalityType, channel string) (*CognitiveEvent, error) {
	b.mu.RLock()
...
func (b *ModalityBus) Act(intent *CognitiveIntent) (*EncodedOutput, error) {
	b.mu.RLock()
...
func (b *ModalityBus) HUD() map[string]any {
	b.mu.RLock()
	defer b.mu.RUnlock()
...
func (b *ModalityBus) Start(ctx context.Context) error {
	b.mu.RLock()
...
func (b *ModalityBus) Stop(ctx context.Context) error {
	b.mu.RLock()
	order := make([]ModalityType, len(b.order))
	copy(order, b.order)
	b.mu.RUnlock()
...
func (b *ModalityBus) Health() map[ModalityType]ModuleStatus {
	b.mu.RLock()
	defer b.mu.RUnlock()
...
func (b *ModalityBus) logEvent(ev BusEvent) {
	b.mu.Lock()
	defer b.mu.Unlock()
